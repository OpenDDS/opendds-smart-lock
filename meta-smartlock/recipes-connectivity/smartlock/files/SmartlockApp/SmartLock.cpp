...
// The green led (gpio 11) is on when it is unlocked and yellow (gpio 10) when it is locked. Red (gpio 09 is not used)
//  9 GPIO10 (SPI_MOSI) Yellow     gpioset gpiochip0 10=1 
// 23 GPIO11 (SPI_CLK)  Green      gpioset gpiochip0 11=1

const unsigned int YELLOW_LED = 10; 
const unsigned int GREEN_LED  = 11;

static gpiod_chip *chip = nullptr;
static gpiod_line *lineLOCKED = nullptr;
static gpiod_line *lineUNLOCKED = nullptr;

void lock_indicator_init() {
  const char *chipname = "/dev/gpiochip0";
  chip = gpiod_chip_open_by_name(chipname);
  if (chip != nullptr) {
    lineLOCKED = gpiod_chip_get_line(chip,YELLOW_LED);
    lineUNLOCKED = gpiod_chip_get_line(chip, GREEN_LED);
    // Open LED lines for output
    if (gpiod_line_request_output(lineLOCKED, "smartlock", 0) < 0) {
      ACE_ERROR((LM_ERROR,
                  ACE_TEXT("ERROR: %N:%l: gpiod_line_request_output() -")
                  ACE_TEXT(" request line as output failed!\n")));

    }
    if (gpiod_line_request_output(lineUNLOCKED, "smartlock", 0) < 0) {
      ACE_ERROR((LM_ERROR,
                  ACE_TEXT("ERROR: %N:%l: gpiod_line_request_output() -")
                  ACE_TEXT(" request line as output failed!\n")));
    }
...
void lock_indicator_shutdown() {
  if (lineLOCKED != nullptr) {
    gpiod_line_release(lineLOCKED);
    lineLOCKED = nullptr;
  }
  if (lineUNLOCKED != nullptr) {
    gpiod_line_release(lineUNLOCKED);
    lineUNLOCKED = nullptr;
  }
  if (chip != nullptr) {
...
void lock_indicator_clear() {
  if (lineUNLOCKED != nullptr && lineLOCKED != nullptr) {
    if (gpiod_line_set_value(lineUNLOCKED, 0) < 0) {
      ACE_ERROR((LM_ERROR,
                  ACE_TEXT("ERROR: %N:%l: gpiod_line_set_value() -")
                  ACE_TEXT(" set line output failed!\n")));
    }
    if (gpiod_line_set_value(lineLOCKED, 0) < 0) {
      ACE_ERROR((LM_ERROR,
                  ACE_TEXT("ERROR: %N:%l: gpiod_line_set_value() -")
                  ACE_TEXT(" set line output failed!\n")));
    }
...
void lock_indicator_lock_when_locked(const SmartLock::lock_t& lock) {
  if (lineUNLOCKED != nullptr && lineLOCKED != nullptr) {
    if (lock.locked()) {
      if (gpiod_line_set_value(lineUNLOCKED, 0) < 0){
        ACE_ERROR((LM_ERROR,
      ...
      if (gpiod_line_set_value(lineLOCKED, 1) < 0){
        ACE_ERROR((LM_ERROR,
                    ACE_TEXT("ERROR: %N:%l: gpiod_line_set_value() -")
                    ACE_TEXT(" set line output failed!\n")));
      }
      ...
      if (gpiod_line_set_value(lineUNLOCKED, 1) < 0){
        ACE_ERROR((LM_ERROR,
                    ACE_TEXT("ERROR: %N:%l: gpiod_line_set_value() -")
                    ACE_TEXT(" set line output failed!\n")));
      }
      ...
      if (result == DDS::RETCODE_OK) {
        ...
        if (error == DDS::RETCODE_OK) {
          ...
          if (info.valid_data) {
            ...
            lock_indicator_lock_when_locked(message.lock());
...
void cleanup() {
  ...
  if (role == kSmartLock) {
    lock_indicator_clear();
  }
  lock_indicator_shutdown();
